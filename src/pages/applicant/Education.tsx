...
import {
  fetchQualifications,
  setEducationalData,
} from "../../store/slices/educationalDetailsSlice";
...
const {
 ...
 useEffect(() => {
   dispatch(fetchQualifications({ page: 1, pageSize: 50, limit: 1000 }));
 }, [dispatch]);

  console.log("Qualifications from Redux:", qualification);
...
console.log("initial education", initialValues);
  const validation: any = useFormik({
    enableReinitialize: true,
    initialValues: {
  qualification: initialQualificationValue,
  specialization: initialValues?.specialization || educationalData?.specialization || "",
  passingYear: initialValues?.passingYear || educationalData?.passingYear || "",
  cgpa: initialValues?.cgpa || educationalData?.cgpa || "",
  collegeName: initialValues?.collegeName || educationalData?.collegeName || "",
},
    validationSchema: EducationApplicantSchema,
    ...
    onSubmit: (data) => {
      ...
      dispatch(setEducationalData(payload)); // <-- Save to Redux

      onNext(payload);
      setLoading(false);
    },
    onReset: () => {
      dispatch(setEducationalData(validation.values)); // <-- Save to Redux on back
      onBack();
    },
  });
  return (
