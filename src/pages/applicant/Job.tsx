...
const JobDetailsForm = ({ onNext, onBack, initialValues }: any) => {
  ...
  useEffect(() => {
    if (appliedSkills.length === 0) {
      dispatch(fetchAppliedSkills());
    }
    if (currentDesignations.length === 0) {
      dispatch(fetchCurrentDesignations());
    }
  }, [dispatch]);

  // Initialize form values from Redux store
  useEffect(() => {
    if (initialLoad && jobData.appliedSkills) {
      const selectedSkills = appliedSkills
        .filter((skill:any) => jobData.appliedSkills.includes(skill.skills))
        .map((skill:any) => ({
          label: skill.skills,
          value: skill._id,
        }));
  ...
  }, [appliedSkills, jobData, initialLoad]);

  const designationOptions = currentDesignations.map((item:any) => ({
    label: item.designation,
    value: item._id,
  }));

  const skillOptions = appliedSkills.map((item:any) => ({
    label: item.skills,
    value: item._id,
  }));
  ...
  // const initialAppliedRoleValue =
  //   designationOptions.find(
  //     (appliedRole:any) => appliedRole.label === initialValues.appliedRole
  //   )?.value || "";
  ...
  const validation: any = useFormik({
    ...
    initialValues: {
      ...
      meta: initialValues?.meta || {},
      ...jobData,
      currentCompanyDesignation: jobData.currentCompanyDesignation || "",
      appliedRole: jobData.appliedRole || "",
      lastFollowUpDate: jobData.lastFollowUpDate
        ? moment(jobData.lastFollowUpDate).format("YYYY-MM-DD")
        : "",
    },
    validationSchema: jobApplicantSchema,
    // onSubmit: (data: any) => {
    ...
    onSubmit: (data) => {
      ...
      dispatch(setJobData(updatedData));
      onNext();
    },
  });
  ...
  const handleMultiSkill = (selected: any[]) => {
  ...
  const handleRoleChange = async (selectedOption: any) => {
    ...
    if (roleId && !roleSkills[roleId]) {
      await dispatch(fetchRoleSkills(roleId));
    }
  };
  const getRoleSkills = () => {
    const roleId = validation.values.appliedRole;
    ...
    return skills.flatMap((roleSkill:any) =>
      roleSkill.skill
        .map((skillId: string) => {
          const skill = appliedSkills.find((s:any) => s._id === skillId);
          return skill ? { label: skill.skills, value: skill._id } : null;
        })
        .filter(Boolean)
    );
  };
            ...
            {loading ? (
              ...
              <Row className="mb-4">
                ...
                {validation.values.appliedRole &&
                  ...
                  validation.values.appliedRole !== "Na" && (
                    <div className="mb-4">
                      {getRoleSkills().map((skill:any, index:any) => (
                        <div className="flex-wrap space-x-2 d-flex">
                          <Col
                            xs={12}
                            sm={2}
                          ...
                          >
                            <BaseInput
                              name={`meta.${skill.value}`}
                              type="text"
                              label={`${skill.label} Experience`}
                              placeholder={`meta.${skill.value}`}
                              value={
                                validation.values.meta?.[skill.value] || ""
                              }
                              handleChange={(e) => {
                                const val = e.target.value;
                                if (
                              ...
                              touched={validation.touched.meta?.[skill.value]}
                              error={validation.errors.meta?.[skill.value]}
                            />
                          </Col>
                        </div>
                      ))}
