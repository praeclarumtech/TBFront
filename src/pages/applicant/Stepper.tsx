...
const StepperForm = () => {
  ...
  useEffect(() => {
    if (id) {
      dispatch(fetchApplicantDetails({ id, module: moduleName }));
    }
  }, [id, moduleName, dispatch]);
  ...
  const handleBack = (data: any) => {
    dispatch(setFormData(data));
    dispatch(setStep(Math.max(0, currentStep - 1)));
  };
              ...
              <Box sx={{ mt: 2 }}>
                {currentStep === steps.length ? (
                  <div>
                    ...
                    <Button onClick={() => dispatch(setStep(0))}>Reset</Button>
                  </div>
                ) : (
                  <>
                    {currentStep === 0 && (
