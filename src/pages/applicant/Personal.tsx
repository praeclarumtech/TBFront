...
import {
  ...
  CheckExistingApplicant,
  setPersonalData,
} from "../../store/slices/personalDetailsSlice";
...
const {
  ...
  const [whatsappError, setWhatsappError] = useState("");
    ...
    const minDateOfBirth = moment().subtract(100, "years").format("YYYY-MM-DD");
    const formattedDateOfBirth = personalData?.dateOfBirth
...
useEffect(() => {
  if (initialValues) {
    dispatch(setPersonalData(initialValues));
  }
    ...
    states.find((state) => state.label === capitalizeWords(initialValues.state))
     ...
     const validation = useFormik({
       ...
       initialValues: {
         ...
         whatsappNumber: personalData?.phone?.whatsappNumber || "",
         phoneNumber: personalData?.phone?.phoneNumber || "",
         ...
         currentCity: initialCityValue,
         currentAddress: personalData.currentAddress || "",
       ...
       onSubmit: async (data: any) => {
         const cityLabel =
           cities.find((c) => c.value === data.currentCity)?.label ||
           data.currentCity;
         const stateLabel =
         ...
         onNext(payload);
        console.log("personal form",payload);
       },
     });
  
  console.log("initivalue", initialValues);
  
  const checkExistingField = async (field: string, value: string) => {
    try {
      const params: any = { [field]: value };
      const response = await dispatch(CheckExistingApplicant(params)).unwrap();

      if (response?.data?.exists) {
        const msg = response?.message || `This ${field} is already in use.`;
        if (field === "email") setEmailError(msg);
        if (field === "phoneNumber") setPhoneNumberError(msg);
        if (field === "whatsappNumber") setWhatsappError(msg);
        return msg;
      } else {
        if (field === "email") setEmailError("");
        if (field === "phoneNumber") setPhoneNumberError("");
        if (field === "whatsappNumber") setWhatsappError("");
        return "";
      }
    } catch (error) {
      errorHandle(error);
  ...
  const handleCancel = () => {
    navigate(
      module === "import-applicant" ? "/import-applicants" : "/applicants"
    );
  };
